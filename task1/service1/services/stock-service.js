...
class StockService {
    async createStorage(productId, shopId, amount) {
        ...
        const dbproduct = await prisma.product.findFirst({ where: { id: productId } });
        ...
        logAction(storage.shop_id, storage.product.plu, Actions.CREATE, `Storage ${storage.id} has been created`);
    ...
    async getStorages(filter) {
        const storages = await prisma.storage.findMany({
            where: {
                ...
                shop_id: filter.shop_id || undefined,
                
                amount: {
                   gte: filter.amount_from || undefined,
                   lte: filter.amount_to || undefined
                }
            }
        });
    ...
    async deleteStorage(productId, shopId) {
        const storage = await prisma.storage.findFirst({ where: { product_id: productId, shop_id: shopId }, include: { product: true } });
    ...
    async createOrder(productId, shopId, amount) {
        ...
        logAction(order.shop_id, order.product.plu, Actions.CREATE, `Order ${order.id} has been created`);
    ...
    async deleteOrder(id) {
        const order = await prisma.order.findFirst({ where: { id }, include: { product: true, shop: true } });

        if (!order) {
            throw createError("Order is not found", 404);
        }

        const dbstorage = await prisma.storage.findFirst({ where: { product_id: order.product.id, shop_id: order.shop.id } });
